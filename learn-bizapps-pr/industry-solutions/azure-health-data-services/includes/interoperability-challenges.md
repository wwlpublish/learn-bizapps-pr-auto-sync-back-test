...
While FHIR, as a standard enables interoperability for better patient outcomes, it is not a replacement IT system for payors, providers, research or pharmaceutical organizations. FHIR does not replace electronic health record (EHR) systems or provider billing systems. FHIR enables the data that needs to be exchanged by these systems by calling FHIR service.
...
- **Securely de-identified data** can be applied for research, clinical, and operational insights.
