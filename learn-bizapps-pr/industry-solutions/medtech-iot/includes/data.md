...
In the left column, we see example messages that might be generated by an IoT device such as a smart watch or heart monitor. The organization requesting this IoT data requires only the HeartRate measurement from the first message; but Systolic and Diastolic blood pressure information is requested from the second message.
