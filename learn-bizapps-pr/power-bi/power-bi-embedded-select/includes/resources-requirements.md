...
Buying the right product SKU is a matter of optimization. Too few resources can result in slow performance and, worst, an inability for Power BI to render analytics. Too many resources can result in wasted money.
...
To reduce cost, you need to undergo capacity planning. The capacity planning process involves establishing a baseline to understand typical resource usage and workload activity. We recommend that you buy the product SKU that can handle the baseline workload. Then, you can adopt elasticity strategies that temporarily scale up the capacity in response to increases in app usage. You can temporarily scale Premium or Power BI Embedded capacities by using different techniques.
