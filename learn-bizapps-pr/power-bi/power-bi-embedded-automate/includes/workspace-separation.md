...
Alternatively, there's the single multi-customer database model. In this case, your solution achieves separation with a single workspace comprising a set of Power BI artifacts that are shared across all tenants. RLS roles, which are defined in the datasets, securely filter the data to ensure organizations only ever see their data.
...
> [!NOTE]
> It isn't necessary to create a new service principal for each tenant. However, we recommend creating a service principal profile for each tenant workspace to achieve the highest possible isolation. For more information, see .
