When provisioning a Power Pages site, the most important choices to consider are the audience, workload, and choosing a specific site template that would best align with the particular business requirements.

Several site templates are available that can be provisioned. These templates will accelerate the configuration of a site based on the intended audience and workload.

![Screenshot of site templates that are available for provisioning.](../media/portal-templates.png)

If you are using Microsoft Dynamics 365 apps such as Dynamics 365 Sales or Dynamics 365 Service, you have a choice of additional Power Pages templates:
...
Each Dynamics 365 site template includes a particular set of features designed to accelerate solution development that targets the selected audience.

> [!NOTE]
> The template selection defines a specific set of features. If a site is provisioned within a Dataverse environment with Dynamics 365 apps installed, specific features from the other templates can be added later, as required.
...
| Customer self-service | Partner, Customer | This template provides the ability for the site users to search knowledge articles, submit cases, and participate in discussion forums to resolve issues. |
| Employee self-service | Employee | This site allows employees to access a centralized knowledge article and to also submit cases. |
| Partner | Partner, Customer, Field Agent | Choose this template to build a site where external partners can manage and collaborate on accounts and opportunities. Add-ons are available for Dynamics 365 Field Service or Dynamics 365 Project Service. |
| Field Service | Customer | This site enables self-service scheduling capabilities for your customers. Requires Dynamics 365 Field Service. |
| Customer portal | Enterprise B2B | The Dynamics 365 Supply Chain Management Customer Portal is a template that provides site access to Dynamics 365 Supply Chain Management data by using dual-write Microsoft Dataverse tables. |
...
- Ability to configure the portal by using the Power Pages design studio.
...
| Feature | Customer self-service | Partner | Employee self-service | Modern Community | Community |
|---------|-----------------------|---------|---|---|---|
| Portal base | • | • | • | • | • |
| Forums | • | • | • | • | • |
| Ideas | | | | • | • |
